...
func _ready():
	...
	for emitter in switch_emitters:
		...
		if not switch_component:
			print(
				"Error: no switch component found in ", emitter.name, "in switch hub: ", self.name
			)
			continue
	...
	for connection in switch_receivers:
		...
		if not switch_component:
			print(
				"Error: no switch component found in ", receiver.name, "in switch hub: ", self.name
			)
			continue
...
func update_activation_sum():
	...
	for switch in switch_emitters:
		nb_activated_switches += int(
			(
				switch.get_node("SwitchComponent").inversed_switch
				!= switch.get_node("SwitchComponent").is_activated
			)
		)  # based on the true/false matrix
